...
export async function execute(_id, params) {
    ...
    if (rpcName) {
        const rpcParams = params.rpc_params ?? {};
    ...
    const inFilters = params.in_filters ?? {};
    const ilikeFilters = params.ilike_filters ?? {};
    const orFilter = params.or_filter;
    const notNull = params.not_null ?? [];
